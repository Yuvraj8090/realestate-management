<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="mt-2 text-3xl font-semibold text-slate-900">Edit {{ $property->title }}</h2>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('properties.update', $property) }}" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="rounded-full bg-slate-900 px-6 py-3 text-sm font-semibold text-white">Save changes</button>
                </div>
            </form>
        </div>
    </div>
